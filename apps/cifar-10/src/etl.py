...
def main(from_date=None, augment=False):
    ...
    metrics = {"timestamp": current_time.isoformat(), "dataset": "CIFAR10", "stage": "etl"}

    # ── Transform ─────────────────────────────────────────────────
    # CIFAR-10: 3-channel RGB, mean/std per channel
    base_transform = [
        transforms.ToTensor(),
    ...
    try:
        ...
        trainset = torchvision.datasets.CIFAR10(RAW_PATH, train=True,  download=True, transform=transform)
        testset  = torchvision.datasets.CIFAR10(RAW_PATH, train=False, download=True, transform=transform)
        ...
        metrics.update({
            ...
            "image_shape":      str(tuple(train_images.shape[1:])),  # (3, 32, 32)
        ...
        run_config = {"from_date": from_date, "image_size": 32, "augmentation": augment}
