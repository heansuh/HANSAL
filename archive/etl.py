...
from perun import monitor as perun_monitor

from config import RAW_PATH, PROCESSED_PATH, METRICS_DIR
from utils import save_metrics, collect_codecarbon_metrics
...
def main():
    ...
    try:
        ...
        def load_and_save(dataset, prefix):
        ...
        train_images, train_labels, load_t, save_t = load_and_save(trainset, "training")
        test_images,  test_labels,  _,      _      = load_and_save(testset,  "test")
        ...
        metrics.update({
            ...
            "throughput_sps":   round(n_samples / total_time, 2),
        ...
        save_metrics(metrics, "etl_benchmark")
